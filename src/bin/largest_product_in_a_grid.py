...
for x in range(20):
    for y in range(20):
        ...
        product_xy = 1
        neg_product_xy = 1
        for offset in range(adjacend):
            if x + adjacend <= 20:
                product_x *= matrix[y][x + offset]
            if y + adjacend <= 20:
                product_y *= matrix[y + offset][x]
            if x + adjacend <= 20 and y + adjacend <= 20:
                product_xy *= matrix[y+offset][x+offset]
            if x - adjacend + 1 >= 0 and y + adjacend <= 20:
                neg_product_xy *= matrix[y+offset][x-offset]

        product = max(product_x, product_y, product_xy, neg_product_xy)
        if product > p_max:
            p_max = product
            p_max = product
